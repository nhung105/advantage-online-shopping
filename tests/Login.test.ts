...
test("login screen contains enough elements in different browsers", async ({
  page,
}) => {
...
test.describe("Verify all log in elements on different screen sizes", () => {
  screenSizes.forEach(({ width, height }) => {
    test(`Login elements are visible on ${width} x ${height}`, async ({
      page,
    }) => {
      await loginPage.setScreenSizes(width, height);
      await expect(loginPage.username).toBeVisible();
      await expect(loginPage.password).toBeVisible();
      await expect(loginPage.rememberMeCheckbox).toBeVisible();
      await expect(loginPage.SignInButton).toBeVisible();
...
test.describe("Valida popup closes", () => {
  ...
  test("Tab Enter after filling incorrect username & password", async ({
    page,
  }) => {
  ...
  test("log in with a blank Username", async ({ page }) => {
    ...
    expect(await loginPage.getUsernameError()).toBe(
      "Username field is required"
    );
    expect(await loginPage.getPasswordError()).toBe(
      "Password field is required"
    );
  });
});
test("login with an invalid Username and invalid Password", async ({
  page,
}) => {
...
test.describe("login successfully", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    registerPage.createnewaccount.click();
  ...
  test("login with an valid Username and valid Password", async ({ page }) => {
    ...
    await loginPage.SignInButton.click();
    (await page.waitForSelector("#menuUserLink > span")).isVisible();
    const userDisplay = await page
      .locator("#menuUserLink > span")
      .textContent();
    console.log(userDisplay);
    expect(userDisplay).toBe(user.username);
  });
});
