...
test.describe("Happy cases", () => {
  dataTest.forEach(
    ({ name, username, email, password, confirmPassword, agreeToTerm }) => {
      test(`${name}`, async () => {
...
test.describe("Check Validation Field", () => {
  ...
  test("register with existing username", async ({ page }) => {
    const existingUser = dataTest.find(
      (user) => user.username === "clone123gn1dd"
    );
    if (!existingUser) {
      throw new Error("User not found");
    }
    const { username, email, password, confirmPassword, agreeToTerm } =
      existingUser;
    await registerPage.fillRegisterForm({
      username,
      email,
    ...
    await registerPage.submitForm();
    await expect(registerPage.existingUserError).toContainText(
  ...
  test.describe("Verify all footer elements on different screen sizes", () => {
    ...
    screenSizes.forEach(({ width, height }) => {
      test(`Footer icons are visible on ${width} x ${height}`, async ({
        ...
        await expect(registerPage.facebookicon).toBeVisible({ timeout: 30000 });
        await expect(registerPage.twittericon).toBeVisible({ timeout: 30000 });
        await expect(registerPage.linkedinicon).toBeVisible({ timeout: 30000 });
